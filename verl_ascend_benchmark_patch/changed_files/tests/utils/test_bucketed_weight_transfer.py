...
try:
    ...
    def get_torch_device():
        raise RuntimeError("torch is required for accelerator transfer tests")
...
def test_bucketed_weight_transfer_stats_schema_on_cpu():
    source = open("verl/workers/rollout/vllm_rollout/bucketed_weight_transfer.py").read()
    ...
    for key in (
        ...
        '"sync_ms": 0.0',
        "BucketedWeightSender stats: %s",
        "BucketedWeightReceiver stats: %s",
    ):
