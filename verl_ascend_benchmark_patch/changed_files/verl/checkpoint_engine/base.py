...
class CheckpointEngineManager:
    ...
    def __init__(
        ...
        self.last_update_weights_timing: dict[str, float] = {}
    ...
    async def update_weights(self, global_steps: int = None):
        ...
        if self.backend == "naive":
            ...
            record_timing("param_sync/naive_update_ms", start)
            self.last_update_weights_timing = timing
            logger.info("CheckpointEngineManager.update_weights timing: %s", timing)
            return

        # 1. abort and save all unfinished requests for partial rollout
        start = time.perf_counter()
        await asyncio.gather(*[r.abort_all_requests() for r in self.replicas])
        record_timing("param_sync/abort_ms", start)
        ...
        record_timing("param_sync/send_recv_update_ms", start)
