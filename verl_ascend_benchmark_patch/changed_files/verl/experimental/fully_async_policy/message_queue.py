...
class MessageQueue:
    ...
    async def get_samples(self, max_n: int, timeout_ms: int = 1000) -> tuple[list[Any], int] | None:
        """
        Get up to max_n samples from the queue in one actor call.

        The method waits until at least one sample is available or the queue is
        shut down. It preserves the existing termination behavior by returning
        None when the queue is closed and empty. If a None sentinel is found in
        the queue, it is returned as part of the batch and no later samples are
        popped in that call.
        """
        if max_n <= 0:
        ...
        async with self._lock:
            if len(self.queue) == 0 and self.running:
                try:
                    await asyncio.wait_for(self._consumer_condition.wait(), timeout=timeout)
...
class MessageQueueClient:
    ...
    async def get_samples(self, max_n: int, timeout_ms: int = 1000) -> tuple[list[Any], int] | None:
        """Get up to max_n samples from queue in one actor call (async)."""
    ...
    def get_samples_sync(self, max_n: int, timeout_ms: int = 1000) -> tuple[list[Any], int] | None:
        """Get up to max_n samples from queue in one actor call."""
        return ray.get(self.queue_actor.get_samples.remote(max_n, timeout_ms))
