...
class FullyAsyncTrainer(SeparateRayPPOTrainer):
    ...
    async def _get_samples_from_queue(self) -> tuple[None, None] | tuple[int, Any]:
        ...
        # Collect samples in batches to reduce Ray actor RPC pressure.
        ...
        while len(queue_samples) < self.required_samples:
            ...
            result = self.message_queue_client.get_samples_sync(max_n=remaining, timeout_ms=1000)
            get_rpc_count += 1
            ...
            if None in samples:
                none_index = samples.index(None)
                queue_samples.extend(samples[:none_index])
                print(
                    f"[FullyAsyncTrainer] Detected termination signal (None), stopping sample collection. "
                    f"Collected {len(queue_samples)}/{self.required_samples} samples"
                )
        ...
        print(
            ...
            f"mq_len: {queue_len}, get_rpc_count: {get_rpc_count}"
        )

        cloudpickle_load_start = time.time()
        queue_samples = [ray.cloudpickle.loads(x) for x in queue_samples]
        cloudpickle_load_time = time.time() - cloudpickle_load_start
        ...
        batch.meta_info["fully_async/message_queue_get_rpc_count"] = get_rpc_count
    ...
    async def _fit_update_weights(self):
        ...
        if getattr(self.checkpoint_manager, "last_update_weights_timing", None):
            self.logger.log(
                data=self.checkpoint_manager.last_update_weights_timing,
