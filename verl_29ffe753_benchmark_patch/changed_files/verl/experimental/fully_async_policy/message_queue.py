...
class MessageQueue:
    ...
    async def get_samples(self, max_n: int) -> tuple[list[Any], int] | None:
        """Get up to max_n samples from the queue with a single Ray RPC."""
        if max_n <= 0:
...
class MessageQueueClient:
    ...
    async def get_samples(self, max_n: int) -> Any | None:
        """Get up to max_n samples from queue with one actor RPC."""
        future = self.queue_actor.get_samples.remote(max_n)
        return await asyncio.wrap_future(future.future())
    ...
    def get_samples_sync(self, max_n: int) -> Any | None:
        """Get up to max_n samples from queue (sync)."""
        return ray.get(self.queue_actor.get_samples.remote(max_n))
