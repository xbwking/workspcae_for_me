...
class BucketedWeightSender:
    ...
    def _new_stats(self) -> dict[str, int | float | str]:
        return {
            "path": "shm" if self.use_shm else "ipc",
            "bucket_count": 0,
            "bucket_bytes": 0,
            "tensor_count": 0,
            "sender_copy_ms": 0.0,
        ...
        try:
            ...
            async for name, weight in ensure_async_iterator(weights):
                ...
                if offset + weight.nbytes > self.bucket_size and len(bucket_meta) > 0:
                    ...
                    self.stats["sync_ms"] += (time.perf_counter() - sync_start) * 1000
                    metadata_start = time.perf_counter()
                    self.socket.send_pyobj({"bucket_meta": bucket_meta, "is_last": False})
                    self.socket.recv()
                    self.stats["metadata_send_ms"] += (time.perf_counter() - metadata_start) * 1000
                    self.stats["bucket_count"] += 1
                    self.stats["bucket_bytes"] += offset
                    bucket_meta = {}
                    offset = 0
                ...
                self.stats["sender_copy_ms"] += (time.perf_counter() - copy_start) * 1000
            ...
            self.stats["bucket_count"] += 1
            self.stats["bucket_bytes"] += offset
            logger.info("BucketedWeightSender stats: %s", self.stats)
        finally:
            self._cleanup()
    ...
    def _direct_send_large_weight(self, name: str, weight: torch.Tensor):
        ...
        self.socket.recv()
        self.stats["bucket_count"] += 1
        self.stats["bucket_bytes"] += weight.nbytes
        self.stats["tensor_count"] += 1
    ...
    def receive_weights(self, on_bucket_received: callable):
        ...
        try:
            ...
            while True:
                ...
                self.stats["metadata_recv_ms"] += (time.perf_counter() - metadata_start) * 1000
                weights, tensor = [], None
                bucket_bytes = 0
                for name, meta in metadata["bucket_meta"].items():
                    ...
                    if handle is not None:
                        ...
                        self.stats["clone_or_to_device_ms"] += (time.perf_counter() - copy_start) * 1000
                        self.stats["tensor_count"] += 1
                        self.stats["bucket_bytes"] += dtype.itemsize * shape.numel()
                        weights.append((name, tensor))
                        continue
                    size = dtype.itemsize * shape.numel()
                    bucket_bytes = max(bucket_bytes, offset + size)
                    ...
                    weights.append((name, tensor))
                self.stats["bucket_count"] += 1
                self.stats["bucket_bytes"] += bucket_bytes
                ...
                if metadata["is_last"]:
                    break
            logger.info("BucketedWeightReceiver stats: %s", self.stats)
        finally:
            self._cleanup()
